...
class ImageObjectDetector:
    ...
    def draw_boxes_on_image(self, image_path, boxes, labels, scores, threshold=0.4):
        ...
        for box, label, score in zip(boxes, labels, scores):
            ...
            text_size = cv2.getTextSize(label_score, cv2.FONT_HERSHEY_SIMPLEX, 0.4, 1)[
                0
            ]
        ...
        results_df = pd.DataFrame(
            {"ID": ids, "Item and Probability": items, "Probability": probabilities}
        )
    ...
    def detect_and_draw_boxes(self, image_path, threshold=0.5, iou_threshold=0.75):
        ...
        return self.draw_boxes_on_image(
            image_path, final_boxes, final_labels, final_scores, threshold
        )
